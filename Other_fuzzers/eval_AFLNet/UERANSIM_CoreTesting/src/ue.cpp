...
static nr::ue::UeConfig *ReadConfigYaml()
{
    ...
    if (yaml::HasField(config, "StateLearner"))
    {
        std::string family = yaml::GetString(config["StateLearner"], "family");
        std::string addr = yaml::GetString(config["StateLearner"], "addr");
        uint16_t port = yaml::GetInt32(config["StateLearner"], "port");
        result->slConf = StateLearnerConf{family, addr, port};
    }
