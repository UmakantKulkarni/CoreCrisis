...
void update_state_aware_variables(struct queue_entry *q, u8 dry_run)
{
  if (getenv("AFLNET_DEBUG")) 
    ACTF("%s response_buf_size=%d, response_bytes=%p\n",
         __func__, response_buf_size, response_bytes);
  khint_t k;
  int discard, i;
  state_info_t *state;
  unsigned int state_count;
...
{
  if (getenv("AFLNET_DEBUG")) ACTF("%s enter net_ip=%s, net_port=%d", __func__, net_ip, net_port);
  int n;
  u8 likely_buggy = 0;
  ...
  if (getenv("AFLNET_DEBUG")) ACTF("%s run %s", __func__, cleanup_script);
  ...
  if (getenv("AFLNET_DEBUG")) ACTF("%s calling connect ...", __func__);
  if(connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
    ...
    if (n== 1000) {
      if (getenv("AFLNET_DEBUG")) ACTF("%s connect failed", __func__);
      close(sockfd);
      return 1;
    }
  }
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    if (getenv("AFLNET_DEBUG")) ACTF("%s sending message %d ...", __func__, messages_sent);
    ...
    if (n != kl_val(it)->msize) {
      if (getenv("AFLNET_DEBUG"))
        ACTF("%s incomplete message sent n=%d, kl_val(it)->msize=%d", 
             __func__, n, kl_val(it)->msize);
...
HANDLE_RESPONSES:
  ...
  if (getenv("AFLNET_DEBUG")) ACTF("%s sending SIGTERM signal", __func__);
  if (terminate_child && (child_pid > 0)) kill(child_pid, SIGTERM);

  //give the server a bit more time to gracefully terminate
  /*
   * NOTE: Since the child process is creating using fork, after sending SIGTERM, there will be a 
   * remaining zombie process until we fetch the exit status with waitpid(), which is called right 
   * after returning from this function. The following while loops forever as errno is EAGAIN (11) 
   * after calling kill on the zobmie process.
   */
  // while(1) {
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  if (corpus_read_or_sync) {
    ...
    if (getenv("AFLNET_DEBUG")) ACTF("%s q->region_count=%d", __func__, q->region_count);
    ck_free(buf);
...
EXP_ST void init_forkserver(char** argv) {
  ...
  if (!forksrv_pid) {
    ...
    if (getenv("AFLNET_DEBUG") == NULL) dup2(dev_null_fd, 1);
    if (getenv("AFLNET_DEBUG") == NULL) dup2(dev_null_fd, 2);
...
static u8 run_target(char** argv, u32 timeout) {
  if (getenv("AFLNET_DEBUG")) ACTF("%s enter", __func__);
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    if (!child_pid) {
      ...
      if (getenv("AFLNET_DEBUG"))
        ACTF("%s pid=%d, execv(\"%s\", argv)", __func__, child_pid, target_path);
      execv(target_path, argv);
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    if (getenv("AFLNET_DEBUG")) ACTF("%s waitpid(child_pid=%d)", __func__, child_pid);
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:e:P:KEq:s:RFc:l:")) > 0)
    ...
    switch (opt) {
      ...
      case 'P': /* protocol to be tested */
        ...
        } else if (!strcmp(optarg, "5GC")) {
          extract_requests = &extract_requests_5gc;
          extract_response_codes = &extract_response_codes_5gc;
